...
export async function registerAllTools(server: McpServer, auth: Record<string, string>) {
  ...
  const buildStyledEmail = (extraHeaders: string[], subject: string, body: string, customHtml?: string) => {
    const html =
      ...
      `<tr><td style="background:#0e7490;padding:20px 28px;">` +
      `<span style="font-size:16px;font-weight:bold;color:#ffffff;letter-spacing:0.5px;">Multimate AI</span>` +
      `<span style="float:right;font-size:11px;color:#a5f3fc;padding-top:3px;">AI-Powered Assistant</span>` +
      `</td></tr>` +
      `<tr><td style="background:#ffffff;padding:28px;border-left:1px solid #e2e8f0;border-right:1px solid #e2e8f0;">` +
      `<div style="font-size:15px;line-height:1.7;color:#1e293b;white-space:pre-wrap;">${escapeHtml(body).replace(/\n/g, "<br/>")}</div>` +
      `</td></tr>` +
      `<tr><td style="background:#ffffff;padding:20px 28px;border-left:1px solid #e2e8f0;border-right:1px solid #e2e8f0;border-top:1px solid #e2e8f0;">` +
      `<span style="font-size:11px;color:#94a3b8;">— Sent Via MultimateAIAgent</span>` +
      `</td></tr>` +
      `<tr><td style="background:#0f172a;padding:14px 28px;text-align:center;">` +
      `<span style="font-size:11px;color:#64748b;">Generated by Multimate AI Agent</span>` +
      `</td></tr>` +
      `</table></td></tr></table></body></html>`;

    const boundary = `multimate_${Date.now().toString(16)}_${Math.random().toString(16).slice(2, 10)}`;
    const raw = [
      ...extraHeaders,
      `Subject: ${subject}`,
      "MIME-Version: 1.0",
