  ...
  try {
    ...
    switch (tool) {
      case "google_docs_create": {
        ...
        if (content) {
          await google.docs({ version: "v1", auth: googleAuth }).documents.batchUpdate({ documentId, requestBody: { requests: [{ insertText: { location: { index: 1 }, text: content } }] } });
        }
      ...
      case "google_docs_read": {
        ...
        if (!res.data?.body?.content) { result = "The document is empty or cannot be read."; break; }
      ...
      case "google_docs_delete_file": {
        const { document_id } = args as any;
        await google.drive({ version: "v3", auth: googleAuth }).files.delete({ fileId: document_id });
      ...
      case "google_docs_edit": {
        ...
        for (const block of content_blocks) {
          let start = block.startIndex ?? currentDocEndIndex; if (start <= 0) start = 1;
          ...
          if (block.operation !== "DELETE") {
            ...
            if (block.bulletPreset === "bullet" || block.bulletPreset === "numbered") { if (nextBulletStartIndex === null) { nextBulletStartIndex = start; nextBulletPreset = block.bulletPreset; } }
            else if (nextBulletStartIndex !== null) { requests.push({ createParagraphBullets: { range: { startIndex: nextBulletStartIndex, endIndex: start }, bulletPreset: nextBulletPreset === "numbered" ? "NUMBERED_DECIMAL_ALPHA_ROMAN" : "BULLET_DASHED_CIRCLE_SQUARE" } }); nextBulletStartIndex = null; nextBulletPreset = null; }
          }
        }
