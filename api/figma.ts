    ...
    switch (tool) {
      case "figma_read_file": {
        ...
        const simplifyNode = (node: any): any => ({
          ...
          children: node.children ? node.children.map(simplifyNode) : undefined,
        });
        result = JSON.stringify({
          name: data.name, lastModified: data.lastModified, version: data.version,
          document: simplifyNode(data.document),
        });
        break;
      }
      ...
      case "figma_read_comments": {
        ...
        result = JSON.stringify(data.comments.map((c: any) => ({
          id: c.id, message: c.message, user: c.user?.handle, createdAt: c.created_at,
          resolved: c.resolved_at !== null, clientMeta: c.client_meta,
        })));
        break;
      }
      ...
      case "figma_get_styles": {
        ...
        result = JSON.stringify({
          styles: data.meta?.styles?.map((s: any) => ({
            name: s.name, styleType: s.style_type, key: s.key, description: s.description,
          })) ?? [],
        });
        break;
      }
      case "figma_get_selection": {
        result = JSON.stringify({
          message: "Use the Multimate Figma Plugin to get selected nodes. The plugin must be running in Figma desktop.",
          instruction: "Open the Multimate Figma Bridge plugin in Figma desktop, select the nodes you want to work with, then ask again.",
        });
        break;
      }
      default:
